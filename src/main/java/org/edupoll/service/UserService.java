...
//		log.info("req.profile exist = {}", request.getProfile() != null);
	...
	// 비밀번호 변경 서비스 메서드
	public void updatePassword(String principal, @Valid UpdatePasswordRequest req) {
		User found = userRepository.findByEmail(principal).get();

		if (found.getPassword().equals(req.getCurrentPassword())) {

			found.setPassword(req.getNewPassword());
